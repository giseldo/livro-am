...
\begin{figure}
	...
	\begin{tikzpicture}
		\draw
		node[draw] at (0,2) (tipo) {Tipo da variável}
		node[draw] at (-3,0) (quan) {Quantitativo}
		node[draw] at (3,0) (quali) {Qualitativo};
		\draw[->] (tipo) -- (quan);
		\draw[->] (tipo) -- (quali);
	\end{tikzpicture}
	\label{fig:tipodado}
...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		node[draw] at (0,2) (tipo) {Tipo da variável}
		node[draw] at (-3,0) (quan) {Quantitativo}
		node[draw] at (5,0) (quali) {Qualitativo}
		node[draw] at (-6,-2) (int) {Intervalar}
		node[draw] at (0,-2) (raz) {Razão}
		node[draw] at (8,-2) (nom) {Nominal}
		node[draw] at (2,-2) (ord) {Ordinal};
		\draw[->] (tipo) -- (quan);
		\draw[->] (tipo) -- (quali);
		\draw[->] (quan) -- (raz);
		\draw[->] (quan) -- (int);
...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		node[draw] at (5,0) (quali) {Qualitativa}
		node[draw] at (-6,-2) (dis) {Discreta}
		node[draw] at (0,-2) (con) {Continua}
		node[draw] at (7,-2) (bin) {Binaria}
		node[draw] at (3,-2) (pol) {Policotômica};
		\draw[->] (tipo) -- (quan);
		\draw[->] (tipo) -- (quali);
		\draw[->] (quan) -- (dis);
		\draw[->] (quan) -- (con);
